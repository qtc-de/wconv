...
class SecurityIdentifier:
    ...
    def __init__(self, binary: bytes, check_length: bool = True) -> None:
        ...
        '''
        self.raw = binary
        self.parsed = parse_binary(binary, check_length)
    ...
    def pretty_print(self, end: str = '\n') -> None:
        ...
        Returns:
            None
        '''
        formatted = self.format_sid()
        name = get_well_known(formatted) or KNOWN_SIDS.get(formatted)

        print_blue('[+] ', end='')
        print_yellow(self.format_sid(), end='')

        if name:
            print_magenta(f' ({name})', end='')
    ...
    def format_sid(self) -> str:
        '''
        Converts the SecurityIndentifier to a SID string.
